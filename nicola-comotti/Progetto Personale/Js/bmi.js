async function calcolaBMI() {
    const tabellaBmi = await fetch('Utils/bmi.json').then(res => res.json());
    ...
    document.getElementById('pesoMin').innerText = eval(pesoMin).toFixed(1);
    document.getElementById('pesoMax').innerText = eval(pesoMax).toFixed(1);
